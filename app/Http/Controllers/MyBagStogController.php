...
class MyBagStogController extends Controller
{
    private const TIENDA_ID=5;
    private const TIENDA_NAME='mybagstog';
    private const TIENDA_DIR='mybagstog';
    ...
    public function index(){
        ...
        foreach ($categorias as $categoria) {
            $producto = Product::where('category_id', $categoria->id)->first();
    ...
    public function category($category_id){
        ...
        $products=Product::where('category_id',$category_id)->orderBy('order_by', 'asc')->paginate(9);
    ...
    public function categorySlug($slug){
        ...
        $products=Product::where('category_id',$category->id)->orderBy('order_by', 'asc')->paginate(9);
        return view(self::TIENDA_DIR.'.category',[
            'category'=>$category,
            'products'=>$products,
        ]);
    ...
    public function productSlug($slug){
        ...
        /*foreach ($product->questions as $data) {
            dd($data->answers);    
         }
         */
    ...
    public function search(SearchRequest $request){
        ...
        $count_products=0;
        $products = Product::where('name','LIKE',"%$query%")->orwhere('key','LIKE',"%$query%")->orwhere('key','LIKE',"%$query%")->orwhere('keywords','LIKE',"%$query%")->get();
