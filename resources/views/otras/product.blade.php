...
{{ contVisitaProducto($product->id) }}
	<nav aria-label="breadcrumb">
		<ol class="breadcrumb">
			...
			@if($category->slug)
				<li class="breadcrumb-item"><a href="{{ route('otras.store.category.slug',$category->slug) }} ">{{ $category->name }}</a></li>
			@else
				<li class="breadcrumb-item"><a href="{{ route('otras.store.category',$category->id) }} ">{{ $category->name }}</a></li>
			@endif
		</ol>
</nav>
    ...
    <div class="row justify-content-center">
        <div class="col-md-10">
			...
			<div class="row">
		        ...
		        <div class="col-md-6">
					@if($product->slug)
					<div class="fb-like" data-href="https://abbaster.com/otras/producto/{{$product->slug}}" data-layout="standard" data-action="like" data-size="small" data-show-faces="true" data-share="true"></div>
					@endif
					...
					@if(!$product->status)
						@if($stock_exhibition)
							<form action="/shopping-cart/addToCart" method="post">
								...
								<button type="submit" class="btn btn-otras btn-block">Agregar al Carro</button>
							</form>
						@endif
					@else
        	...
        	<div class="row">
				...
				<div class="col-md-7">
		            ...
		            <div class="row">
						<div class="col-md-12">
							<div class="fb-comments" data-href="https://abbaster.com/otras/producto/{{$product->slug}}" data-numposts="5"></div>
						</div>
					</div>
