        ...
        <div class="card">
            ...
            <div class="card" v-for="customer in  customers" :key="customer.id">
              <div class="card-body">
                <div class="row">
                    <div class="col">
                      <div v-for="nuevo in news" :key="nuevo.id">
                        <span class="badge badge-success" v-if="(nuevo.mail==customer.mail)">Nuevo</span>
                      </div>
                      <img src="/images/perfil.png" alt="user" class="img-thumnail">
                    </div>
                    <div class="col">
                        <dl class="row">
                    ...
                    <div class="col">
                        <button type="button" class="btn btn-info btn-block" @click="abrirModal('customer','actualizar', customer)"><i class="fa fa-trash"></i> Editar</button>
                        <button type="button" class="btn btn-danger btn-block" @click="eliminar(customer.id)"><i class="fa fa-trash"></i> Eliminar</button>
                        <br>
                        <div v-for="nuevo in news" :key="nuevo.id">
                          <button type="button" class="btn btn-success btn-block" v-if="(nuevo.mail==customer.mail)" @click="marcar_enterado(nuevo.id)"><i class="fa fa-check"></i> Marcar notificación leida</button>
                        </div>
    ...
    export default {
      ...
      computed:{
    	...
    	methods:{
          ...
          let me=this;
          var url = '/customers/get?page='+page+'&buscar='+buscar+'&criterio='+criterio;
        ...
        eliminar(id){
            ...
            marcar_enterado(id){
                ...
                swalWithBootstrapButtons.fire({
                  title: '¿Marcar la notificación como leida?',
                  icon: 'warning',
                  showCancelButton: true,
                  confirmButtonText: 'Aceptar',
                  cancelButtonText: 'Cancelar',
                  ...
                  if (result.value) {
                    ...
                    let me=this;
                    axios.put('/admin/customers/read-ntf',{
                        'id': id
                    }).then(function (response){
                        me.getClientes(1,'','nombre');
                        swalWithBootstrapButtons.fire(
